...
void clear_file() {
    // Mở file với chế độ ghi (write) và không có dữ liệu (ios::trunc)
    std::ofstream outfile("Data_Game.txt", std::ios::trunc | std::ios::out); 

    if (outfile.is_open()) {
        // File đã được mở thành công, mọi nội dung sẽ bị xóa
        outfile.close(); 
        // Đóng file sau khi xóa nội dung
    } else {
        std::cout << "Unable to open file <DataTable.txt> for clearing." << std::endl;
    }
}
